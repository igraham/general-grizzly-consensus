   ...
   public void actionPerformed(ActionEvent e)
   {
       ...
       else if (e.getID() == GGCGlobals.INSTANCE.MESSAGE_EVENT_ID)
       {
           if (messages.getText() != null && messages.getText().length() > 0)
               messages.setText(messages.getText() + "\n" + ((GGCConnection)e.getSource()).getRawSocket().getRemoteSocketAddress() + ": " + e.getActionCommand());
           else
               messages.setText(((GGCConnection)e.getSource()).getRawSocket().getRemoteSocketAddress() + ": " + e.getActionCommand());
       }
   }
}